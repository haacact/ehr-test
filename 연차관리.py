...
# 🚀 [추가] 연차촉진서 전자서명 및 열람 로그를 관리하는 함수
@st.cache_data(ttl=30)
...
# 🚀 [추가] 2차 연차촉진 대상자에게 보내는 전자문서 알림 메일
...
# --- 🏠 내 연차 신청/현황 ---
    ...
    # 🚀 [추가] 로그인한 대상자가 연차촉진서 전자서명 대상인지 스캔
...
# --- 🌐 [관리자] 전사 통합 관리 ---
    ...
    with tab_promo:
        ...
        st.dataframe(promo_status[['팀', '직급', 'ID', '이름', '실잔여', '서명상태', '발송일시', '서명일시']].rename(columns={'ID':'사번', '실잔여':'미계획잔여일'}), hide_index=True, use_container_width=True)
        
        if st.button("🚀 위 명단 전체에게 2차 연차촉진서 발행 및 안내 메일 발송", type="primary"):
            with st.spinner("대상자를 스캔하고 메일을 발송 중입니다..."):
                new_logs = df_logs.copy()
                send_cnt = 0
                now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                
                for _, emp in eligible_emp.iterrows():
                    emp_id = str(emp['ID'])
                    exist = new_logs[new_logs['Emp_ID'] == emp_id]
                    
                    # 이미 서명을 완료한 사람은 재발송하지 않음
                ...
                if send_cnt > 0 or not new_logs.equals(df_logs):
                    ...
                    st.warning("새로 발송할 대상자가 없습니다. (모두 서명완료 상태)")
